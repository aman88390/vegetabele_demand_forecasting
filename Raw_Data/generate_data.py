...
#     # ✅ Ensure data does NOT go beyond 31 Dec 2025
#     end_date = datetime(2025, 11, 30)
#     # We want exactly `days` days, ending at end_date
#     start_date = end_date - timedelta(days=days - 1)
#     dates = [start_date + timedelta(days=i) for i in range(days)]
    
#     # 1. Define random high-demand event days (weddings/festivals)
#     event_dates = set()
#     for _ in range(15):
#         rand_day = np.random.randint(0, days)
#         event_dates.add((start_date + timedelta(days=rand_day)).strftime('%Y-%m-%d'))

#     # 2. Item Profiles (Fruits higher base price than vegetables)
#     items = {
#         'Apple':  {'base_qty': 45,  'base_price': 120, 'type': 'Fruit'},
#         'Banana': {'base_qty': 100, 'base_price': 40,  'type': 'Fruit'},
#         'Onion':  {'base_qty': 80,  'base_price': 30,  'type': 'Staple'},
#         'Potato': {'base_qty': 90,  'base_price': 25,  'type': 'Staple'},
#         'Tomato': {'base_qty': 70,  'base_price': 40,  'type': 'Staple'},
#     }

#     # 3. Simple veg seasonality (internal only, not saved as columns)
#     #    In-season veg: cheaper & higher demand than off-season
#     veg_in_season = {
#         'Onion':  [11, 12, 1, 2],  # winter onions cheap & abundant
#         'Potato': [11, 12, 1, 2],
#         'Tomato': [3, 4, 5, 6],    # summer tomatoes
#     }

#     data = []

#     print("Generating RAW data with events, weather, seasonality, and noise...")
...
#         # Weather: still affects normal days strongly
...
#             # --- SEASONALITY FOR VEGETABLES (internal) ---
#             if profile['type'] == 'Staple':
...
#             # Fruits: keep base season behavior (they already have higher base price)
#             # You can extend similar seasonality for fruits later if you want.

#             # --- WEEKEND EFFECT ---
#             if is_weekend:
#                 # Fruits sell more on weekends than staples
#                 qty *= 1.25 if profile['type'] == 'Fruit' else 1.10
...
#                 if weather == 'Rain':
#                     qty *= 0.85   # slight drop but still high
#                 elif weather == 'Sunny':
...
#                 price_shock = np.random.uniform(0.9, 1.1)
#                 price *= price_shock
...
#     # Build DataFrame
#     columns = ["Date", "Item", "Quantity_Sold", "Price", "Is_Event", "Is_Weekend", "Weather"]
#     df = pd.DataFrame(data, columns=columns)

#     # --- RAW DATA CORRUPTION TO MAKE IT REALISTIC ---
#     print("Injecting missing values, wrong values, and duplicates to make data truly raw...")

#     # 1. Random missing values (~3%)
#     for col in ["Quantity_Sold", "Price", "Weather", "Item"]:
#         mask = np.random.rand(len(df)) < 0.03
#         df.loc[mask, col] = np.nan

#     # 2. Wrong numeric values (~1%)
#     wrong_idx = df.sample(frac=0.01, random_state=42).index
#     df.loc[wrong_idx, "Quantity_Sold"] = np.random.choice([-5, -10, 9999])
...
#     print("✅ RAW dataset ready: 'sales_data_raw.csv' (dates limited to ≤ 2025-12-31)")
